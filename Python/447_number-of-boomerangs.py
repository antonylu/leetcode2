...
class Solution(object):
    def numberOfBoomerangs(self, points):
        ...
        :rtype: int
        """
        # Approach #2, brute-force, use dict to speed up
        # take one point as the center of circles
        # all the other points are distributed on different level of circles
        # if any of those points have the same distance form the center, they are located on the same level of circle
        # if there are n points on the same circle, how many numberOfBoomerangs for the middle point?
        # a permutation (order matters), first is chosen from n points, the second point is chosen from (n-1) points
        # 
    ...
    def numberOfBoomerangs2(self, points):
        ...
        # distance of  p1(x1,y1) and p2(x2,y2), dis(p1,p2) = sqrt ((x2-x1)^2+(y2-y1)^2)
        #
        # There are C(n,3) combinations
        # 3 possibilities of a touple
        #  0: no middle point
        #  2: one middle point
...
if __name__ == '__main__':
    ...
    for i in range(len(tc)):
        print(s.numberOfBoomerangs(tc[i]))
        #assert(s.numberOfBoomerangs(tc[i]) == an[i])
